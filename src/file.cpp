...
off_t File::pos() {
	return (file) ? ftell(file) : off_t(-1);
}
...
size_t File::writeInt(int32_t value) {
	if(!file)
		return -1;
...
size_t File::writeInt64(int64_t value) {
	if(!file)
		return -1;
...
size_t File::writeChar(const char *source, size_t n) {
...
size_t File::write(vector<unsigned char> &v) {
	if(v.empty())
		return  0;
	if(!file)
		return -1;
