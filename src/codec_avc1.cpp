...
typedef struct H264DSPContext {
  ...
  /* loop filter */
  void (*h264_v_loop_filter_luma)(uint8_t* pix /*align 16*/,
                                  ptrdiff_t stride,
                                  int alpha,
                                  int beta,
                                  int8_t* tc0);
  void (*h264_h_loop_filter_luma)(uint8_t* pix /*align 4 */,
                                  ptrdiff_t stride,
                                  int alpha,
                                  int beta,
                                  int8_t* tc0);
  void (*h264_h_loop_filter_luma_mbaff)(uint8_t* pix /*align 16*/,
                                        ptrdiff_t stride,
                                        int alpha,
                                        int beta,
                                        int8_t* tc0);
  /* v/h_loop_filter_luma_intra: align 16 */
  void (*h264_v_loop_filter_luma_intra)(uint8_t* pix,
                                        ptrdiff_t stride,
                                        int alpha,
                                        int beta);
  void (*h264_h_loop_filter_luma_intra)(uint8_t* pix,
                                        ptrdiff_t stride,
                                        int alpha,
                                        int beta);
  void (*h264_h_loop_filter_luma_mbaff_intra)(uint8_t* pix /*align 16*/,
                                              ...
                                              int alpha,
                                              int beta);
  void (*h264_v_loop_filter_chroma)(uint8_t* pix /*align 8*/,
                                    ptrdiff_t stride,
                                    int alpha,
                                    int beta,
                                    int8_t* tc0);
  void (*h264_h_loop_filter_chroma)(uint8_t* pix /*align 4*/,
                                    ptrdiff_t stride,
                                    int alpha,
                                    int beta,
                                    int8_t* tc0);
  ...
  // h264_loopfilter.c
  void (*h264_loop_filter_strength)(int16_t bS[2][4][4],
                                    uint8_t nnz[40],
                                    int8_t ref[2][40],
                                    int16_t mv[2][40][2],
                                    int bidir,
  ...
  void (*h264_idct_add16)(uint8_t* dst /*align 16*/,
                          const int* blockoffset,
                          int16_t* block /*align 16*/,
                          int stride,
                          const uint8_t nnzc[15 * 8]);
  void (*h264_idct8_add4)(uint8_t* dst /*align 16*/,
                          const int* blockoffset,
                          int16_t* block /*align 16*/,
                          int stride,
                          const uint8_t nnzc[15 * 8]);
  void (*h264_idct_add8)(uint8_t** dst /*align 16*/,
                         const int* blockoffset,
                         int16_t* block /*align 16*/,
                         int stride,
                         const uint8_t nnzc[15 * 8]);
  void (*h264_idct_add16intra)(uint8_t* dst /*align 16*/,
                               const int* blockoffset,
                               int16_t* block /*align 16*/,
                               int stride,
                               const uint8_t nnzc[15 * 8]);
...
typedef void (*h264_chroma_mc_func)(uint8_t* dst /*align 8*/,
                                    uint8_t* src /*align 1*/,
                                    ptrdiff_t srcStride,
                                    int h,
                                    int x,
...
typedef struct H264Picture {
  ...
  int mmco_reset;         /**< MMCO_RESET set this 1. Reordering code must
                                                           not mix pictures before and
                             after MMCO_RESET. */
  int pic_id;             /**< pic_num (short -> no wrap version of pic_num,
                                                           pic_num & max_pic_num; long ->
                             long_pic_num) */
  int long_ref;           ///< 1->long term reference 0->short term reference
  int ref_poc[2][2][32];  ///< POCs of the frames/fields used as reference
                          ///< (FIXME need per slice)
  int ref_count[2][2];    ///< number of entries in ref_poc         (FIXME need
                          ///< per slice)
...
typedef struct H264PredContext {
  void (*pred4x4[9 + 3 + 3])(uint8_t* src,
                             const uint8_t* topright,
                             ptrdiff_t stride);
  void (*pred8x8l[9 + 3])(uint8_t* src,
                          int topleft,
                          int topright,
                          ptrdiff_t stride);
...
#define PART_NOT_AVAILABLE -2
  ...
  uint8_t*
      list_counts;  ///< Array of list_count per MB specifying the slice type

  /* 0x100 -> non null luma_dc, 0x80/0x40 -> non null chroma_dc (cb/cr), 0x?0 ->
  ...
  int has_slice;  ///< slice NAL is found in the packet, set by
                  ///< decode_nal_units, its state does not need to be preserved
                  ///< outside h264_decode_frame()
   ...
   * SEI_PIC_STRUCT_TOP_BOTTOM and SEI_PIC_STRUCT_BOTTOM_TOP indicate interlaced
   * frames. However, soft telecined frames may have these values. This is used
   * in an attempt to flag soft telecine progressive.
   */
  ...
  /**
   * We have seen an IDR, so all the following frames in coded order are
   * correctly decodable.
   */
...
#define FRAME_RECOVERED_SEI (1 << 1)
  ...
  int ref2frm[MAX_SLICES][2]
             [64];  ///< reference to frame number lists, used in the loop
                    ///< filter, the first 2 are for -2,-1
} H264Context;
#else
...
void H264sps::parseSPS(const uint8_t* data, int size) {
  ...
  p += 6;
  if (cnt != 1) {
    Log::debug << "Not supporting more than 1 SPS unit for the moment; might "
                  "fail horribly.\n";
  }
  for (int i = 0; i < cnt; i++) {
    // nalsize = AV_RB16(p) + 2;
...
class NalInfo {
 public:
  ...
  bool getNalInfo(const H264sps& sps,
                  uint32_t maxlength,
...
void NalInfo::print(int indentation) {
  ...
  Log::debug << indent << "Length         : " << length
             << ((length < 8 + 4 || length > MaxAVC1Length + 4) ? " (incorrect)"
                                                                : "")
             << '\n';
  Log::debug << indent << "Ref idc        : " << ref_idc
             << ((unsigned(ref_idc) > 3) ? " (incorrect)" : "") << '\n';
  Log::debug << indent << "Nal type       : " << nal_type
             << ((unsigned(nal_type) > 0x1f) ? " (incorrect)" : "") << '\n';
  Log::debug << indent << "First mb       : " << first_mb << '\n';
  Log::debug << indent << "Slice type     : " << slice_type
             << ((unsigned(slice_type) > 9) ? " (incorrect)" : "") << '\n';
...
bool NalInfo::getNalInfo(const H264sps& sps,
                         uint32_t maxlength,
                         const uint8_t* buffer) {
  // Re-initialize.
  clear();
  ...
  for (unsigned int i = 0; i < len; i++) {
    ...
    if (i + 2 < len && buffer[i] == 0 && buffer[i + 1] == 0 &&
        buffer[i + 2] == 3) {
  ...
  if (slice_type > 9) {
    Log::debug << "Invalid slice type (" << slice_type
               << "), probably this is not an avc1 sample.\n";
    return false;
  }
...
Match Codec::avc1Search(const unsigned char* start, int maxlength) {
  ...
  for (int offset = 0; offset < maxlength - 8; offset++) {
    ...
    if (nal_type > 21)
      continue;
    // this looks like it might be a packet. might want to do a more thorough
    // check.
    match.offset = offset;
    match.chances = 10;
    return match;
...
Match Codec::avc1Match(const unsigned char* start, int maxlength) {
  ...
  // First 4 bytes is the length the the packet and it's expected not to be
  // bigger than 16M
  if (start[0] != 0) {
  ...
  // See 7.4.1.2.4 Detection of the first VCL NAL unit of a primary coded
  // picture
  ...
  H264Context* h =
      static_cast<H264Context*>(context->priv_data);  // context->codec->
  const SPS* hsps = NULL;
  if (h) {
    ...
    if (!hsps && h->ps.sps_list[0]) {
      // Use first SPS.
      hsps =
          reinterpret_cast<const SPS*>(h->ps.sps_list[0]->data);  // may_alias.
    }
  }
  if (!hsps) {
  ...
  while (true) {
    ...
    Log::debug << "Nal type: " << info.nal_type << " Nal ref idc "
               << info.ref_idc << endl;
