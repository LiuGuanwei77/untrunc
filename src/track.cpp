...
// Track.
Track::Track()
    : trak(nullptr), type{0}, timescale(0), duration(0), id(-1), nsamples(0) {}
...
bool Track::parse(Atom* t, AVCodecParameters* codecpar) {
  cleanUp();

  if (!t) {
    Log::error << "Missing 'Container for an individual Track or stream' atom "
                  "(trak).\n";
    return false;
  }
  trak = t;
  ...
  if (!mdhd)
    throw string(
        "Missing 'Media Header' atom (mdhd): Unknown duration and timescale");
  ...
  if (!default_time && !default_size && times.size() != sample_sizes.size()) {
    ...
    Log::debug << "Time offsets: " << times.size()
               << " Size offsets: " << sample_sizes.size() << '\n';
  }
  // assert(times.size() == sizes.size());
  /*	if(!default_time && times.size() != sample_to_chunk.size()) {
                  Log::info << "Mismatch between time offsets and
     sample_to_chunk offsets.\n"; Log::debug << "Time offsets: " << times.size()
  ...
  "\n"; return true;
          ...
          if(avcodec_open2(codec.context, codec.codec, NULL) < 0) {
                  throw string("Could not open codec: ")
                                  + ((codec.context->codec &&
  codec.context->codec->name)? codec.context->codec->name : "???");
          }
  }*/
  if (!codec.InitCodec(codecpar)) {
    return false;
  }
...
void Track::writeToAtoms() {
  ...
  if (!elst)
    Log::error << "Missing 'Edit List' atom (elst).\n";
  else {
    uint8_t version = elst->readUInt8(0);
    int entries = elst->readInt(4);
    if (entries > 0) {
      if (version == 1) {
        elst->writeInt64(duration, 8);
	  } else {
        elst->writeInt(duration, 8);
	  }
	}
  }

  // Avc1 codec writes something inside stsd.
  // In particular the picture parameter set (PPS) in avcC (after the sequence
  // parameter set). For avcC see:
  // <http://jaadec.sourceforge.net/specs/ISO_14496-15_AVCFF.pdf>. For PPS
  // see:
  // <https://www.iitk.ac.in/mwn/vaibhav/Vaibhav%20Sharma_files/h.264_standard_document.pdf>.
  // For PPS  see:
  // <http://last.hit.bme.hu/download/firtha/video/h264/Iain%20E.%20Richardson%20-%20H264%20(2nd%20edition).pdf>.
  // I have found out in shane,banana,bruno and nawfel that
  // pic_init_qp_minus26 is different
  ...
  if (codec.name == "avc1") {
    ...
    if (stsd)
      stsd->writeInt(pps[SHANE], 122);  // a bit complicated to find.... find
                                        // avcC... follow first link.
  }
#endif
}
...
void Track::getChunkOffsets(Atom* t) {
  ...
  if (nchunks == 0)
    Log::debug
        << "Missing both 'Chunk Offset' atoms (stco & co64) or no chunks!";
  ...
  for (int i = 0; i < entries; i++) {
    ...
    for (int k = first_chunk; k < last_chunk; k++) {
      ...
      if (default_size) {
        ...
        chunks[k].size =
            chunks[k].nsamples * default_size * codec.pcm_bytes_per_sample;
...
void Track::saveSampleToChunk() {
  ...
  if (!codec.pcm && default_chunk_nsamples == 0) {
    default_chunk_nsamples = 1;
    Log::debug << "Don;t know how to deal with variable sample size and "
                  "variable number of samples per chunk. Trying with 1.\n";
  }

  if (default_size ==
      0) {  // video might put more samples in the same chunk, even a constant
            // number, default_chunk_nsamples might be != 0
    // TODO we should distinguish those cases.
    // so if we don;t have a default number of samples per chunk we save one
    // sample per chunk.
