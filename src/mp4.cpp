...
void Mp4::close() {
	...
	for (Track& track : tracks) {
		if (track.codec.context) {
			avcodec_free_context(&track.codec.context);
		}
	}
...
void Mp4::simulate(Mp4::MdatStrategy strategy, int64_t begin) {
	...
	std::string *codecs = new std::string[tracks.size()];
...
bool Mp4::parseTracks() {
	...
	for(unsigned int i = 0; i < traks.size(); ++i) {
		Track track;
		track.parse(traks[i], context->streams[i]->codecpar);
...
//		cout.flush();
	...
	if(matches.size() < 4) {//too few packets.
	    Log::error << "Too few packets" << endl;
		return false;
	}
