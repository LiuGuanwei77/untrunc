...
#ifndef NDEBUG
	off_t end = file.pos();
	//std::cout << "atom name: " << name << ", end-begin: " << end-begin << ", length: " << length << std::endl;
	...
	if(name == string("mvhd") || name == string("mdhd")) {
		...
		for(int i = 0; i < nentries; i++) {
			...
			if(type == string("mp4a")) {
				...
				const int extensionSize = readInt(off + 86);
				...
				if(extension == string("avcC")) {
					...
					int eoff = 94;
					uint8_t* sbuffer = new uint8_t[extensionSize+1];
